...
void parse_net_options(list *options, network *net) {
    ...
    // 如果 batch 较大, 需要分成 mini_batch 给 gpu, 但是还是一个 batch 更新一个 weights
    net->batch /= subdivs;
    net->batch *= net->time_steps;
    // 一个 mini_batch 处理图片的张数
    ...
    // 表示 lr warm up 的步数, learning_rate * pow(iterations / burn_in, power)
    ...
    if (net->policy == STEP) {
        ...
        // 每达到一个 steps, lr 缩小 scale 倍, 即 lr=lr*scale
...
network *parse_network_cfg(char *filename) {
    ...
    // unclear: size-1 是因为开始的 [net] 不计算在内吗?
...
list *read_cfg(char *filename) {
    ...
    char *line;
    int nu = 0;
    // adam: replace options with sections
